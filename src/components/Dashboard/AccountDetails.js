...
import { detailProjectAccount } from "../../redux/slice/accountSlice";
...
const {account, loading, error} = accountSlice
...
dispatch(detailProjectAccount(id))
  ...
  return (
    <>
    {error && <h2 className="text-center text-red-500 mt-5">{error}</h2>}
      {/* Account Summary */}
      <section
        className="py-20 xl:pt-24 xl:pb-32 bg-sky-100"
        style={{
      ...
      ? 
      <h2 className="text-center text-black-500 mb-5 text-3xl">No transactions to display</h2>
       : loading ? <Loading /> : <TransactionList transactions={account?.data?.transactions}/>}
    </>
  );
};
